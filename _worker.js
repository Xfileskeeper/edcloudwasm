    ...
    return new Response(new ReadableStream({
        async start(controller) {
            ...
            try {
                ...
                if (isGrpc) {
                    while (true) {
                        const {done, value} = await reader.read(new Uint8Array(sessionBuffer, used, 16384));
                        if (done) break;
                    ...
                    while (true) {
                        !state.tcpWriter && (offset = used);
                        const {done, value} = await reader.read(new Uint8Array(sessionBuffer, offset, 16384));
                        if (done) break;
                        sessionBuffer = value.buffer;
